...
import { createProject, PROJECT_TYPES, type ProjectType } from '../project-templates.js'
...
export const projectCreateTool = defineTool({
  name: 'project-create',
  description:
    '创建一个工程。支持多种工程类型：空白文件夹、项目管理工程、代码类工程（Java、Python、Vue、DSH Plugin）、知识类工程、探索类工程、复制类工程。' +
  ...
  parameters: {
    ...
    projectType: {
      ...
      description:
        '工程类型。可选值：empty（空白文件夹）、project-management（项目管理工程）、' +
  ...
  async execute(args) {
    const result = await createProject({
      ...
      projectType: args.projectType as ProjectType,

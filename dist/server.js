...
function route(method, url) {
    if (method === 'GET' && url === '/api/bookmarks')
        return handleListBookmarks;
    if (method === 'POST' && url === '/api/bookmarks')
        return handleAddBookmark;
    if (method === 'DELETE' && url.startsWith('/api/bookmarks/'))
        return handleRemoveBookmark;
    if (method === 'POST' && url === '/api/open')
        return handleOpenBookmark;
    if (method === 'POST' && url === '/api/create-project')
        return handleCreateProject;
    if (method === 'GET' && url === '/api/select-folder')
        return handleSelectFolder;
    if (method === 'GET' && url === '/api/project-types')
        return handleProjectTypes;
...
let HTML = '';
export async function startServer(port = 17319) {
